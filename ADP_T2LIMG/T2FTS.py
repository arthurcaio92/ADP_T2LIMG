...
class FuzzySet(object):
    ...
    def __init__(self, domain, umf=zero_mf, umf_params=[], lmf=zero_mf, lmf_params=[],nome='nome',check_set=False):
        ...
        'Minimmum value in which membership degree is > 0'
        #self.actual_domain_min = umf_params[0]
        
        'Maximmum value in which membership degree is > 0'
        #self.actual_domain_max = umf_params[-1]
...
class Type2Model():
    ...
    def ADP_center_part_desvio(self,cloud_info,numero_de_sets,mf_type):
        ...
        """
        Cria os conjuntos fuzzy usando os centros e desvio padrão de cada cloud do ADP
        """
    ...
    def ADP_uneven_center(self,cloud_info,numero_de_sets,mf_type):
        
        """
        Cria os conjuntos fuzzy usando os centros de cada cloud do ADP (como é feito no FCM)
        """
        ...
        centros_y.append(self.dominio_inf)
        centros_y.append(self.dominio_sup)
        centros_y = list(set(centros_y))
        centros_y.sort()
                
        for c in np.arange(1, len(centros_y) - 1):
            if mf_type=='triangular':
                sets_param.append([round(centros_y[c - 1], 3), round(centros_y[c], 3),
                                                 round(centros_y[c + 1], 3)])          
                
        Type2Model.generate_uneven_length_mfs(self, numero_de_sets, mf_type, sets_param)
    ...
    def ADP_center_part_limites(self,cloud_info,numero_de_sets,mf_type):
        ...
        """
        Cria os conjuntos fuzzy usando os centros, valores minimos e valores maximos de cada cloud do ADP
        """
    ...
    def ADP_part_limites_entropia(self,cloud_info,numero_de_sets,mf_type, dados_para_plot_limits_grid):
        
        """
        Usa os centros, valores minimos e valores maximos de cada cloud do ADP para gerar um UoD secundário, e divide 
        esse UoD em sets usando entropia
        
        A ideia e o ADP dividir os dados em X data clouds, e dentro de cada data cloud,, pega-se o mínimo e o máximo 
        e faz um particionamento por entropia desse sub-UoD
        """
        ...
        'Vamos criar um dict com as samples pertencentes a cada cloud'
        
        dict_samples_por_cloud = {}
        
        list_samples_por_cloud = dados_para_plot_limits_grid[-1]
        
        df_dados = pd.DataFrame()
        df_dados['data'] = self.training_data
        df_dados['Cloud'] = list_samples_por_cloud
        
        T = np.unique(list_samples_por_cloud)
        for x in T:
            fatiado = df_dados.loc[(df_dados['Cloud'] == x)]
            fat = fatiado['data']
            dict_samples_por_cloud['%d'%x] = fat
        
        #print(dict_samples_por_cloud.keys())
        
        
        'Define o numero de sets em que cada data cloud sera dividido'
        #secondary_n_sets = int(numero_de_sets * 0.5)
        #secondary_n_sets = numero_de_sets
        
        #print('secondary_n_sets',secondary_n_sets)
        print('numero_de_sets DO ADP',numero_de_sets)
        ...
        from ADP_T2LIMG.ADP_T2LIMG.Partitioners import ENTROPY_part
        
        'Faz o particionamento grid de todos os conjuntos de todos os data clouds'
        
        'Identify each data cloud extrems (from min to max values)'
        for data_cloud in range(1,self.numero_de_sets+1):
        #for data_cloud in dict_samples_por_cloud.keys(): 
                
            dados_para_entropia = dict_samples_por_cloud['%d'%data_cloud]
            
       
            if mf_type == 'triangular':
                
                'O ´processo da entropia demora um pouco mesmo'
                entropy_params = ENTROPY_part(dados_para_entropia, numero_de_sets, mf_type)
                n_of_sets_entropy = len(entropy_params)
                
                #print("N de sets ENTROPIA (antes de sobrepostos)",n_of_sets_entropy)
                    
                'Builds the intervals'
                for x in range(1,n_of_sets_entropy+1):
                    
                    aux = entropy_params[x-1]
                    #print('aux',aux)
                    #intervalos_conjuntos.append(aux)
                    #conj_vec = np.arange(float(aux[0]),float(aux[-1]),0.01)
                    conj_vec = [aux[0],aux[-1]]
                    nome = 'A%d'%numero_nome_set 
                    set_lengths[nome] = (conj_vec,aux,data_cloud)   #key: conjunto / value: length
                    numero_nome_set = numero_nome_set + 1
                    
                    

        #print('ddddddd',set_lengths.keys())
        print("N de sets (antes de sobrepostos)",len(set_lengths))
        
                
        
        '##### VAMOS EXCLUIR SETS TOTALMENTE SOBREPOSTOS #####'        
        resultado = {}
        apagar = {}

        for x in np.arange(1,len(set_lengths)+1):
                        
            conjunto = set_lengths['A%d'%x][0]
            #print('conjunto',conjunto)
            dict_auxiliar = set_lengths
            cont = 1   #variable for identifying the set in the 2 for loop
            res = []
            erase = []
            for conj_comp,ignore,d_cloud in dict_auxiliar.values():   #Iterate over all set lengths
                        
                if conjunto[0] > conj_comp[0] and conjunto[-1] < conj_comp[-1]:
                    erase.append(x)                  
                cont = cont + 1
            resultado['A%d'%x] = res
            apagar['A%d'%x] = erase
        ...
        'Exclui as linhas do df das clouds a serem apagadas'
        for x in sets_del:
            set_lengths.pop('A%d'%x)
            #cloud_info.drop(cloud_info.index[cloud_info['Cloud'] == x], inplace=True)
        
             
        'Defines the new number of sets'
        #cloud_info['Cloud'] = np.arange(1,len(cloud_info['Centros'])+1)
        self.numero_de_sets = len(set_lengths)
        ...
        'Inicializa este dict para onseguir plotar os sets depois. Cria uma lista de sets para cada data cloud'
        sets_por_data_cloud = {}
        
        for n in set_lengths.values():
            cl = n[-1]
            sets_por_data_cloud['%d'%cl] = []
           
        numero_nome_set = 1
                                           
        'Builds each fuzzy set by advancing in the dict: set_lengths '   

            
        for x in set_lengths.values():
            ...
            nome = 'A%d'%numero_nome_set 
            dict_sets['A%d' %numero_nome_set] = FuzzySet(self.domain, tri_mf, [b_esq, topo_tri, b_dir,1],tri_mf, [b_esq+fou_left, topo_tri, b_dir-fou_right, 0.9],nome = nome)
            
            'Armazena o conjunto ao data cloud que ele pertence no dict sets_por_data_cloud'
            cloud = x[-1]
            sets_por_data_cloud['%d'%cloud].append(dict_sets['A%d' %numero_nome_set])
            
            numero_nome_set = numero_nome_set + 1
            
         
        #print('sets_por_data_cloudddddd',sets_por_data_cloud)   
        #for c in sets_por_data_cloud.values():
            #IT2FS_plot_limites_grid(*c,mf_shape = 'triangular')
        

        #self.plot_limites_grid(dados_para_plot_limits_grid, sets_por_data_cloud)
        
        self.dict_sets = dict_sets
        
        
    def ADP_part_limites_grid(self,cloud_info,numero_de_sets,mf_type, dados_para_plot_limits_grid):
        
        """
        Usa os centros, valores minimos e valores maximos de cada cloud do ADP para gerar um UoD secundário, e divide 
        esse UoD em sets igualmente espaçados
        
        A ideia e o ADP dividir os dados em X data clouds, e dentro de cada data cloud,, pega-se o mínimo e o máximo 
        e faz um particionamento grid desse sub-UoD
        """
        ...
        print('secondary_n_sets',secondary_n_sets)
        print('numero_de_sets',numero_de_sets)
        ...
        'Identify each data cloud extrems (from min to max values)'
        for data_cloud in range(1,self.numero_de_sets+1):    
            corte = cloud_info.loc[cloud_info['Cloud'] == data_cloud]
            
            sub_dominio_inf = round(float(corte['Min']),2)
            sub_dominio_sup =  round(float(corte['Max']),2)
            
            #print('data_cloud',data_cloud,'sub_dominio_inf',sub_dominio_inf,'sub_dominio_sup',sub_dominio_sup)
            
            pontos = sub_dominio_sup - sub_dominio_inf
            
            #print('pontos',pontos)
            
            'Points where each fuzzy sets begins in the grid partitioning'
            intervalo_entre_set = pontos/(secondary_n_sets+1) 
            
            #print('intervalo_entre_set',intervalo_entre_set)
            
            if mf_type == 'triangular':
                
                'Finds the beggining points of each interval'
                pontos_conj = []
                for x in range(secondary_n_sets+2):
                    pontos_conj.append(sub_dominio_inf + (intervalo_entre_set * x))
                    
                'Builds the intervals'
                #intervalos_conjuntos = []
                for x in range(1,secondary_n_sets+1):
                    aux = [pontos_conj[x-1],pontos_conj[x],pontos_conj[x+1]]
                    #print('aux',aux)
                    #intervalos_conjuntos.append(aux)
                    #conj_vec = np.arange(float(aux[0]),float(aux[-1]),0.01)
    ...
    def plot_limites_grid(self,dados_para_plot_limits_grid, sets_por_data_cloud):
        
        data, centre, IDX = dados_para_plot_limits_grid
        ...
        'para que todos os graficos tenham os mesmos eixos x'
        eixoxxx = np.linspace(self.dominio_inf,self.dominio_sup,10)
                 
        'Plota o grafico com os eixos invertidos'
        #plt.figure()
        plt.figure()
        ...
        legends = None
        title = None
        for dc,sets in sets_por_data_cloud.items():            
