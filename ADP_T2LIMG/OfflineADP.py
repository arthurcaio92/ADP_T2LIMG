...
def plotar_adp(data, centre, IDX):
    ...
    #print('elements_count_ordered',elements_count_ordered)
    #print('maiores_clouds',maiores_clouds)
 
    
    return (cloud_info, lista_centros)


def plotar_adp_NOVO(data, centre, IDX):
    
    plt.plot(data['avg'])
    plt.figure()
    plt.title('ADP Output novo')
    adp_idx = IDX
    centros = centre
    df_serie_temporal = data
    df_serie_temporal['Clouds'] = adp_idx
    ...
    'Constroi dataframe cloud_info com dados de cada data cloud'
    T = np.unique(adp_idx)
    cloud_info = pd.DataFrame()
    lista_max = []
    lista_min = []
...
def ADP(data, granularity=11, distancetype='chebyshev'):
    ...
    for ii in range(1,L):
        #print('lennnnn dist', len(dist))
        #print('ii',ii)
        #print('int(pos[ii-1])',int(pos[ii-1]))
        #print('seq',seq)
    ...
    dados_para_plot_limits_grid = [dados_original, centre, adp_idx]
        
  
    cloud_info, centros = plotar_adp_NOVO(dados_original, centre, adp_idx)
    
    return centros, cloud_info, dados_para_plot_limits_grid
