...
import pandas as pd
import numpy as np
from ADP_T2LIMG.ADP_T2LIMG.Headquarters import T2FTS
            ...
            for mf_type in mfs:
                ...
                for part_param in partition_parameters:
                    ...
                    for lag in orders:
                        ...
                        while (rodar_janelas == 0):
                            
                            print("Dataset:", data_name, "///", "Modelo:", method_part)
                            ...
                            'Se a janela superior é igual ao final da serie temporal, pára esse loop while'
                            
                            if window_sup == len(data):
                                break
                            
                            window_inf = int(window_inf + window_size * increment)
                            window_sup = int(window_sup + window_size * increment)
                            
                            'Se a janela superior é maior que o final da serie temporal, clipa no final da serie e roda somente mais uma vez, porque'
                            'o if window_sup == len(data) vai quebrar o loop while'
                ...
                if diff == 0:  
                    ...
                    name_file = method_part + "_SW" + "_diff_" + "[" + data_name + "]" + "_" + mf_type + "_" + str(partition_parameters[0]) + "a" + str(partition_parameters[-1]) + ".xlsx"      
