...
def SODA_part(data,gridsize):
    ...
    from ADP_T2LIMG.ADP_T2LIMG.SODA import SODA_function 
...
def ADP_part_t_anterior(data,gridsize, distancetype='chebyshev'):
    
    "Usa como input ao ADP a serie temporal (y) e a serie temporal defasada de -1 (y)"
    ...
    from ADP_T2LIMG.ADP_T2LIMG.OfflineADP import ADP
    #from OfflineADP import ADP 
        
    #Shifts the training array -1 instances
    shifted_data = np.delete(data, 0)
    #Delete the last instance of the training data so it has the same length as the shifted array
    data = data[:-1]
    ...
    #dados.insert(1, 'Shifted', shifted_data)
    dados.insert(1, 'Shifted', data)
    
    #print(dados)
        
    centre, cloud_info = ADP(dados,gridsize)   
    ...
    """
    #PLOTA OS CENTROS DOS CLOUDS
    centre_x = []
    centre_y = []
    
    for x in np.arange(0,len(centre)):       
        centre_x.append(centre[x][0])
        centre_y.append(centre[x][1])
        
    plt.figure()
    plt.plot(data,color = 'blue')    
    plt.scatter(centre_x,centre_y,color = 'red')
    """
                         
    return numero_de_sets,cloud_info
    
 
def ADP_part(data,gridsize, distancetype='chebyshev'):
    
    "ADP após modificações feitas pelo Thiago"
	...
	#Make it a two-column dataframe       
    dados = DataFrame(data, columns = ['avg'])
    dados.insert(0, 'n', range(1,len(dados)+1))
            
    centre, cloud_info, dados_para_plot_limits_grid = ADP(dados,gridsize)   
    #centre, idx = ADP(dados)         #DEFAULT = 11
    ...
    return numero_de_sets,cloud_info, dados_para_plot_limits_grid


def ADP_part_antigo(data,gridsize, distancetype='chebyshev'):
    'A distancia pode ser chebyshev, euclidean, cityblock, sqeuclidean ou cosine'
    ...
    from ADP_T2LIMG.ADP_T2LIMG.OfflineADP import ADP_antigo 
    #from OfflineADP import ADP 
    ...
    centre, idx = ADP_antigo(dados,gridsize)   
    #centre, idx = ADP(dados)         #DEFAULT = 11
