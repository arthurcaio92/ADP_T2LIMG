# -*- coding: utf-8 -*-
from ADP_T2LIMG.ADP_T2LIMG.T2FTS import Type2Model,IT2FS_plot,IT2FS_plot_OLD
from ADP_T2LIMG.ADP_T2LIMG.Tools import error_metrics,plot_forecast 
from ADP_T2LIMG.ADP_T2LIMG.Partitioners import SODA_part,ADP_part,ADP_part_antigo,DBSCAN_part,FCM_part,ENTROPY_part,CMEANS_part,HUARNG_part,ADP_part_t_anterior
from ADP_T2LIMG.ADP_T2LIMG.Transformations import Differential
import numpy as np
import pandas as pd
...
def T2FTS(data,method_part,mf_type,partition_parameters,order,diff,training):
    ...
    if method_part == 'chen':
    ...
    elif method_part == 'ADP_part_t_anterior':
        gridsize = partition_parameters
        number_of_sets,cloud_info = ADP_part_t_anterior(training_data, gridsize)
        ...
        gridsize = partition_parameters
        number_of_sets,cloud_info,dados_para_plot_limits_grid = ADP_part(training_data, gridsize)
        modelo.grid_partitioning(number_of_sets, mf_type) 
        #modelo.ADP_uneven_center(cloud_info,number_of_sets, mf_type)
        #modelo.ADP_center_part_limites(cloud_info,number_of_sets, mf_type)
        #modelo.ADP_center_part_desvio(cloud_info,number_of_sets, mf_type)
        
        
    elif method_part == 'ADP_LIMITES_GRID' or method_part == 'ADPT2LIMG' :
        gridsize = partition_parameters
        number_of_sets,cloud_info, dados_para_plot_limits_grid = ADP_part(training_data, gridsize)
        modelo.ADP_part_limites_grid(cloud_info,number_of_sets, mf_type, dados_para_plot_limits_grid)
        number_of_sets = modelo.numero_de_sets #Atualiza o n de sets com todos os sets de cada data cloud   
        #modelo.dados_para_plot_limits_grid = dados_para_plot_limits_grid
        
        
        
    elif method_part == 'ADP_LIMITES_ENTROPIA':
        gridsize = partition_parameters
        number_of_sets,cloud_info, dados_para_plot_limits_grid = ADP_part(training_data, gridsize)
        modelo.ADP_part_limites_entropia(cloud_info,number_of_sets, mf_type, dados_para_plot_limits_grid)
        number_of_sets = modelo.numero_de_sets #Atualiza o n de sets com todos os sets de cada data cloud   
        #modelo.dados_para_plot_limits_grid = dados_para_plot_limits_grid
        
    
                 
    elif method_part == 'ADP_DESVIO':
        gridsize = partition_parameters
        number_of_sets,cloud_info = ADP_part(training_data, gridsize)
        modelo.ADP_center_part_desvio(cloud_info,number_of_sets, mf_type)
        
    elif method_part == 'ADP_CENTROS':
        gridsize = partition_parameters
        number_of_sets,cloud_info = ADP_part(training_data, gridsize)
        modelo.ADP_uneven_center(cloud_info,number_of_sets, mf_type)
    ...
    plot_title = str(modelo.numero_de_sets) + ' partitions'
    """
    hg = {}
    rrr = 0
    for xf,xh in modelo.dict_sets.items():
        if rrr != 5:
            hg[xf] = xh
            rrr =rrr+1
        
       
    IT2FS_plot(*hg.values(),title= plot_title,mf_shape = mf_type)
    """
